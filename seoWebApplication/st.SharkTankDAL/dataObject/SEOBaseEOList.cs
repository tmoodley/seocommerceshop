...
{
    [Serializable()]
    public abstract class SEOBaseEOList<T> : SEOBaseBOList<T>
        where T : SEOBaseEO, new()
    {
        ...
        {
            foreach (SEOBaseEO genericEO in this)
            {
                if (genericEO.DBAction == SEOBaseEO.DBActionEnum.Save)
                {
                ...
                {
                    if (genericEO.DBAction == SEOBaseEO.DBActionEnum.Delete)
                    {
